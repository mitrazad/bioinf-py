...
class Gene(object):
    def __init__(self, dna=None, exon_regions=None):
        ...
        """
        # Logic for random generation if no arguments are provided
        if dna is None and exon_regions is None:
            # Generate a random DNA string (e.g., 100 bases long)
            self._dna = generate_string(100)
            self._exon_regions = None
            self._exons = None
            self._introns = None
            return
